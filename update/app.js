...
main.innerHTML = html;
  ...
  _hwAddManual() {
    ...
    st.lastMsg = badWords.length > 0
      ? '<span style="color:#C62828">⚠️ 有 ' + badWords.length + ' 项不在教材词库中（可能输入有误）：' + badWords.map(w => this._h(w)).join('、') + '，已标红，请核对</span>'
      : '<span style="color:#2E7D32">✅ 已添加 ' + added + ' 项，已通过词库校验</span>';
    input.value = '';
    this._renderHomeworkEditorUI();
  },
...
this.currentView = 'dictation';
  ...
  attachExerciseListeners(exercise) {
    ...
    if (exercise.type === 'listen') {
      ...
      setTimeout(() => this.speakWord(exercise.audioWord), 500);
      document.querySelectorAll('.option-btn').forEach(btn => {
        btn.addEventListener('click', () => this.handleAnswer(btn.dataset.answer, btn));
      });
    }
    ...
    if (exercise.type === 'hearChoose') {
      ...
      setTimeout(() => this.speakWord(exercise.audioWord), 400);
      document.querySelectorAll('.option-btn').forEach(btn => {
        btn.addEventListener('click', () => this.handleAnswer(btn.dataset.answer, btn));
      });
    }
    ...
    if (exercise.type === 'hearSpell') {
      ...
      setTimeout(() => this.speakWord(exercise.audioWord), 300);
  ...
  checkMatch(exercise) {
    ...
    if (isCorrect) {
      ...
      if (remaining.length === 0) {
        ...
        setTimeout(() => this.nextExercise(), 800);
      }
    } else {
  ...
  speakWord(word) {
    this._ttsCancel();
    this._ttsSpeak({ text: word.replace(/\s*\/\s*/g, ', '), language: 'en-US', volume: 1 });
  },

  speakChinese(word) {
    this._ttsCancel();
    this._ttsSpeak({ text: word, language: 'zh-CN', volume: 1 });
  },
...
window.__SERVER_VER = '20260821-1663';